...
//小图切换过程
	function minshow(xtqhnum){
		...
		if(can.mindire==true){
			mingdjl_num = xtqhnum - picminnum + can.min_picnum;
			jq(can.pnum).find('ul li').css('float','left');
			if(picminnum>can.min_picnum){
				mingdjl_w = mingdjl_num > 0 ? -(picminnum - can.min_picnum) * picminw : -xtqhnum * picminw;
				//if(xtqhnum<can.min_picnum/2){mingdjl_w=0;}
